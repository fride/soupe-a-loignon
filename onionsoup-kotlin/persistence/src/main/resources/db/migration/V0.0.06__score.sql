create table loan_application_score
(
    loan_application_id varchar unique primary key,
    score               text not null,
    comments            text[],
    constraint fk_loan_application_score__loan_application foreign key (loan_application_id) references loan_application_id (id)
);
...
select loan_application_id.id          as id,
       ...
       score.comments                  as score_comments
from loan_application_id
         ...
         left join loan_application_score score on score.loan_application_id = loan_application_id.id
    );
